    ...
    <form method="post">
        ...
        <table class="table table-bordered">
            ...
            <tbody id="product-rows">
                <?php foreach ($existingProducts as $i => $product): ?>
                    <tr>
                        <td class="text-center"><?= $i + 1 ?></td>
                        <td>
                            <input type="text" 
                                   name="products[<?= $i ?>][kode_barang]" 
                                   class="form-control form-control-sm" 
                                   value="<?= Html::encode($product->kode_barang_custom) ?>">
                        </td>
                        <td>
                            <input type="text" 
                                   name="products[<?= $i ?>][nama_barang]" 
                                   class="form-control form-control-sm" 
                                   value="<?= Html::encode($product->nama_barang_custom) ?>">
                        </td>
                        <td class="text-center">
                            <button class="btn btn-sm btn-info" 
                                    type="button" 
                                    data-bs-toggle="collapse" 
                                    data-bs-target="#bom-<?= $i ?>">
                                BOM
                            </button>
                    ...
                    <tr class="collapse" id="bom-<?= $i ?>">
                        <td colspan="4">
                            <div class="p-2 border rounded bg-light">
                                <h6 class="mb-2">Detail BOM - Produk #<?= $i + 1 ?></h6>
                                <table class="table table-sm table-bordered mb-0" id="bom-table-<?= $i ?>">
                                    <thead class="table-light">
                                        <tr>
                                    ...
                                    <tbody>
                                        <?php if (!empty($product->bomCustoms)): ?>
                                            <?php foreach ($product->bomCustoms as $j => $bom): ?>
                                                <tr class="bom-row">
                                                    <td class="text-center"><?= $j + 1 ?></td>
                                                    <td>
                                                        <select name="products[<?= $i ?>][bom][<?= $j ?>][barang_id]" 
                                                                class="form-select form-select-sm"
                                                                onchange="updateSatuan(this, <?= $i ?>, <?= $j ?>)">
                                                            <option value="">Pilih Bahan</option>
                                                            <?php foreach ($barangList as $barangId => $barang): ?>
                                                                <option value="<?= $barangId ?>" 
                                                                        data-unit="<?= $barang['unit_id'] ?>"
                                                                        <?= $bom->barang_id == $barangId ? 'selected' : '' ?>>
                                                    ...
                                                    <td>
                                                        <input type="number" 
                                                               name="products[<?= $i ?>][bom][<?= $j ?>][qty]" 
                                                               class="form-control form-control-sm" 
                                                               step="0.01" 
                                                               min="0"
                                                               value="<?= Html::encode($bom->qty_per_unit) ?>">
                                                    </td>
                                                    <td>
                                                        <input type="text" 
                                                               id="satuan-<?= $i ?>-<?= $j ?>" 
                                                               class="form-control form-control-sm" 
                                                               readonly 
                                                               value="<?= Html::encode($bom->barang->unit->nama_unit ?? '') ?>">
                                                    </td>
                                                    <td class="text-center">
                                                        <button type="button" 
                                            ...
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <!-- Default 1 row BOM kosong jika belum ada -->
                                            <tr class="bom-row">
                                                <td class="text-center">1</td>
                                                <td>
                                                    <select name="products[<?= $i ?>][bom][0][barang_id]" 
                                                            class="form-select form-select-sm"
                                                            onchange="updateSatuan(this, <?= $i ?>, 0)">
                                                        <option value="">Pilih Bahan</option>
                                                        <?php foreach ($barangList as $barangId => $barang): ?>
                                                            <option value="<?= $barangId ?>" 
                                                                    data-unit="<?= $barang['unit_id'] ?>">
        ...
        <div class="mt-3">
            ...
            <?= Html::a('Cancel', ['master-pelanggan/view', 'pelanggan_id' => $model->pelanggan_id], ['class' => 'btn btn-secondary']) ?>
        </div>
    </form>
</div>
...
let productCounter = $existingCount;
...
function addProductRow() {
    ...
    const newRow = `
        <tr data-product-index=\"` + newIndex + `\">
            ...
            <td>
                <input type=\"text\" name=\"products[` + newIndex + `][kode_barang]\" class=\"form-control form-control-sm\">
            </td>
            <td>
                <input type=\"text\" name=\"products[` + newIndex + `][nama_barang]\" class=\"form-control form-control-sm\">
            </td>
            <td class=\"text-center\">
                    ...
                    <table class=\"table table-sm table-bordered mb-0\" id=\"bom-table-` + newIndex + `\">
                        ...
                        <tbody>
                            <tr class=\"bom-row\">
                                ...
                                <td>
                                    <input type=\"number\" name=\"products[` + newIndex + `][bom][0][qty]\" class=\"form-control form-control-sm\" step=\"0.01\" min=\"0\">
                                </td>
                                <td>
                                    <input type=\"text\" id=\"satuan-` + newIndex + `-0\" class=\"form-control form-control-sm\" readonly>
                                </td>
                                <td class=\"text-center\">
