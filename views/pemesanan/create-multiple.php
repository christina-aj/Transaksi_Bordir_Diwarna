...
<div class="pc-content">
    ...
    echo $form->field($pemesanan[0], 'barang_id')
        ->dropDownList($dataPost, ['prompt' => 'Pilih Barang', 'id' => 'barang_id']);
    ?>

    <!-- Field User ID dan Info User -->
    <?= $form->field($pemesanan[0], 'user_id')->hiddenInput(['value' => Yii::$app->user->id])->label(false) ?>
    ...
    echo $form->field($pemesanan[0], 'user_info')->textInput(['value' => $user_info, 'readonly' => true, 'label' => 'User']);
    ?>

    <!-- Widget DatePicker untuk Field Tanggal -->
    <?= $form->field($pemesanan[0], 'tanggal')->widget(DatePicker::classname(), [
    ...
    <!-- Field Qty -->
    <?= $form->field($pemesanan[0], 'qty')->textInput() ?>

    <!-- Bagian Tabel Dinamis untuk Multiple Input -->
    <div class="pemesanan-items">
        <h3>Pemesanan</h3>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <?php foreach ($pemesanan as $i => $item): ?>
                    <tr>
                        <td>
...
<script>
    // Fungsi untuk menambah baris pemesanan baru
    $('.add-item').on('click', function() {
        ...
        var template = `<tr>
            <td><select name="Pemesanan[${index}][barang_id]" class="form-control">
                ${optionsHtml}
            </select></td>
            <td><input type="hidden" name="Pemesanan[${index}][user_id]" value="<?= $userId; ?>" /></td> <!-- Eksekusi PHP untuk mengambil user ID -->
            <td><input type="date" name="Pemesanan[${index}][tanggal]" class="form-control" /></td>
            <td><input type="number" name="Pemesanan[${index}][qty]" class="form-control" /></td>
            <td><button type="button" class="btn btn-danger remove-item">Remove</button></td>
        </tr>`;
        $('table tbody').append(template);
    });

    // Fungsi untuk menghapus baris pemesanan
