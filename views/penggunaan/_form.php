    ...
    <div class="card table-card">
        ...
        <div class="card-body mx-4">
                ...
                <tbody id="table-body">
                    <?php foreach ($modelDetails as $index => $modelDetail): ?>
                        <tr>
                            ...
                            <td class="barang-column"><?= $form->field($modelDetail, "[$index]barang_id")->textInput(['readonly' => true])->label(false) ?></td>
                            ...
                            <td><?= $form->field($modelDetail, "[$index]nama_barang")->widget(Typeahead::classname(), [
                                    'options' => [
                                        ...
                                        'id' => "penggunaandetail-{$index}-nama_barang",
                            ...
                            <td>
                                <?= $form->field($modelDetail, "[$index]jumlah_digunakan")->textInput(['placeholder' => '500'])->label(false) ?>
                                <small class="text-muted">dalam gram</small>
                            </td>
            ...
            <?php if ($modelDetail->isNewRecord): ?>
                <?= Html::a('Cancel', ['cancel', 'penggunaan_id' => $modelPenggunaan->penggunaan_id], [
                    'id' => 'cancelButtons',
            ...
            <?php else: ?>
                <?= Html::a('Back', ['view', 'penggunaan_id' => $modelPenggunaan->penggunaan_id], ['class' => 'btn btn-secondary']) ?>
            <?php endif; ?>
        </div>
    </div>
</div>
...
$this->registerJs("
    ...
    $(document).on('click', '.add-row', function() {
        var newRow = `
            <tr>
                ...
                <td>
                    <input type='number' name='PenggunaanDetail[` + rowIndex + `][jumlah_digunakan]' class='form-control' step='any' min='0' placeholder='500'>
                    <small class='text-muted'>dalam gram</small>
                </td>
    ...
    // AUTO-FILL BOM - LANGSUNG GRAM (TIDAK ADA KONVERSI)
    " . (!empty($permintaanId) ? "
    \$(document).ready(function() {
        setTimeout(function() {
            \$.ajax({
                ...
                success: function(res) {
                    if (res.success && res.data.length > 0) {
                        ...
                        \$.each(res.data, function(i, b) {
                            // LANGSUNG PAKAI GRAM DARI BOM (tidak kali 1000)
                            var qtyGram = parseFloat(b.qty);
                            ...
                            var row = '<tr>' +
                                ...
                                '<td><input type=\"number\" name=\"PenggunaanDetail['+i+'][jumlah_digunakan]\" class=\"form-control\" value=\"'+qtyGram+'\" min=\"0\" step=\"any\" required><small class=\"text-muted\">dalam gram</small></td>' +
                        ...
                        \$('.barang-column').hide();
                        toggleAddDeleteButtons();
                        alert('Form akan diisi dengan '+res.data.length+' item dari BOM ('+res.kode_permintaan+'). Qty dalam GRAM. Silakan review dan Save.');
                    } else {
